...
input bool EnableStochastic = false;      // Enable Stochastic indicator
input bool EnableBollingerBands = false;  // Enable Bollinger Bands
input double MaxDrawdownPercentage = 10;  // Maximum drawdown percentage before stopping trading

// Global variables
double MACD_Main, MACD_Signal, EMA_Value, RSI_Value;
int macd_handle, ema_handle, rsi_handle, stochastic_handle, bollinger_handle;
...
int OnInit()
{
    ...
    if (EnableStochastic)
        stochastic_handle = iStochastic(NULL, 0, 14, 3, 3, MODE_SMA, STO_LOWHIGH);
    if (EnableBollingerBands)
        bollinger_handle = iBands(NULL, 0, 20, 0, 2, PRICE_CLOSE);
...
void OnDeinit(const int reason)
{
    ...
    if (EnableStochastic) IndicatorRelease(stochastic_handle);
    if (EnableBollingerBands) IndicatorRelease(bollinger_handle);
