...
#property description "Automated Scalping EA with EMA/RSI Strategy and Price Action"
...
input int MagicNumber = 86110;               // Unique Magic Number
...
input string TradeComment = "AutoScalper"; // Trade comment

// Money Management
input group "Money Management"
input double RiskPercentage = 1.0;           // Risk % per trade (0.5-2%)
input double FixedLotSize = 0.02;            // Fixed lot size (0=auto)
input double MaxDailyLossPercent = 100.0;     // Max daily loss % (0=off)
input double MaxDrawdownPercent = 15.0;     // Max account drawdown % (0=off)
input double MaxDrawdownPerPosition = 10.0; // Maximum drawdown per position in dollars
input double ProfitThreshold = 5.0; // Profit in dollars to activate SL and BE
...
input bool UseTrendLineConfirmation = true; // Use trend line confirmation for entries
...
//+------------------------------------------------------------------+
//| Check for trend line confirmation                                |
//+------------------------------------------------------------------+
bool CheckTrendLineConfirmation()
{
    double currentPrice = iClose(_Symbol, _Period, 0);
    double lastHigh = iHigh(_Symbol, _Period, 1);
    double lastLow = iLow(_Symbol, _Period, 1);

    // For buy signal, check if current price is above a "trend line" created by last high
    if(currentPrice > lastHigh)
        return true; // Trend line confirmation for buy

    // For sell signal, check if current price is below a "trend line" created by last low
    if(currentPrice < lastLow)
        return true; // Trend line confirmation for sell

    return false; // No trend line confirmation
}
...
void CheckForTradeSignal()
{
    ...
    bool buySignal = currentPrice > emaFast && 
                     ...
                     rsi < RsiOverbought &&
                     CheckBullishCandle() &&
                     (!UseTrendLineConfirmation || CheckTrendLineConfirmation());
...
void UpdateBreakEven(ulong ticket)
{
    ...
    // Only set break-even if profit exceeds threshold
