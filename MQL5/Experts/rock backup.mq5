...
input int MagicNumber=58690;
input int MaxOpenPositions=5;
input double LockInProfitThreshold=20;           // $ profit to lock in
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      if(!isProfitsLocked && currentProfit>=LockInProfitThreshold)
      {
         double newSL=(type==POSITION_TYPE_BUY) ? 
                      currentPrice-currentATR*StopLossATRMultiplier : 
                      currentPrice+currentATR*StopLossATRMultiplier;
      ...
      {
         double totalDistance=(type==POSITION_TYPE_BUY) ? 
                             (currentTP-openPrice) : 
                             (openPrice-currentTP);
