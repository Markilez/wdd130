...
//+------------------------------------------------------------------+
//| Open trades (only reversal trades)                               |
//+------------------------------------------------------------------+
void OpenTrades()
{
   ...
   // Ensure no simultaneous trades and only open reversal trades
   if (totalPositions == 0) 
   {
      if (buySignal && CheckTradingAllowed() && iClose(_Symbol, TimeFrame, 0) < resistanceLevel)
      {
         OpenTrade(true, positionSize);
      }
      if (sellSignal && CheckTradingAllowed() && iClose(_Symbol, TimeFrame, 0) > supportLevel)
      {
   ...
   {
      // Open additional positions for strong reversal signals
      if (buySignal && CheckTradingAllowed() && iClose(_Symbol, TimeFrame, 0) < resistanceLevel)
      {
         OpenTrade(true, positionSize);
      }
