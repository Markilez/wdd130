...
input int CooldownPeriod = 60;             // Cooldown period in seconds after closing a trade
...
datetime lastTradeCloseTime; // Store the time of the last closed trade
...
void OnTick()
{
    ...
    // Check if cooldown period has passed since the last trade was closed
    if (TimeCurrent() < lastTradeCloseTime + CooldownPeriod)
    {
        Print("Cooldown period active. No new trades will be opened.");
        return;
    }
    ...
    {
        if (CheckSwingTradeConditions() && CountOpenPositions() < MaxOpenPositions)
        {
            if (ShowEntrySignals)
            {
                DrawEntrySignal();
...
void OpenTrade()
{
    ...
    else
    {
        ...
        Print("Strive and thrive"); // Print the message after a successful trade
        // Store trade details
...
void CloseTrade(ulong ticket)
{
    ...
    else
    {
        ...
        Print("Strive and thrive"); // Print the message after a successful trade close
        lastTradeCloseTime = TimeCurrent(); // Update the last trade close time
    }
}
